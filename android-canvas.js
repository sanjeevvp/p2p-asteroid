  ...
  socket.on("init", function(data) {
    ...
    if (existing) {
      socket.on("game", function(data) {
        ...
        asteroids = serverasteroids;

        particlePool = serverParticlePool;
        asteroidPool = serverasteroidPool;
        bulletPool = serverbulletPool;
      });
    } else {
      starter = true;
    }
...
function particleInit() {
  if (!existing) {
    ...
    particles = [];
    socket.emit("particleInit", { particlePool, particles });
  }
}
...
function bulletInit() {
  if (!existing) {
    ...
    bullets = [];
    socket.emit("bulletInit", { bulletPool, bullets });
  }
}

function asteroidInit() {
  if (!existing) {
    asteroidPool = Pool.create(Asteroid, 40);
    asteroids = [];
    socket.emit("asteroidInit", { asteroidPool, asteroids });
  }
}
